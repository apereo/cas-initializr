...
public class CasOverlayTerraformAwsAppRunnerContributor extends TemplatedProjectContributor {

    public CasOverlayTerraformAwsAppRunnerContributor(final ApplicationContext applicationContext) {
        super(applicationContext, "./terraform/aws", "classpath:overlay/terraform/aws/**");
    }

    @Override
    public void contribute(final Path projectRoot) throws IOException {
        if (getOverlayProjectDescription().isTerraformAwsAppRunner()) {
